...
Copyright (C) 2012 Joseph Amato, Jon Anderson, Tom Carlone, Michael Fagan
  ...
  For questions, contact j.l.amato@gmail.com
 */
...
public:
	...
	// Initiate Homing
	bool initiateHoming(long int setpoint);
...
private:
	...
	bool activateProfileHomingMode(){
		int loopCounter=0;
		while(!VCS_ActivateHomingMode(keyHandle, nodeID, &errorCode)){
			if(loopCounter++>maxCommandAttempts){
				ROS_WARN_STREAM("Could not activate Profile Homing Mode for Node: " <<nodeID<< " : " << getErrorInfo(errorCode));
				return false;
			}
		}
		return true;
	...
	bool setHomingParameter(long int setpoint){
		int loopCounter=0;
		while(!VCS_SetHomingParameter(keyHandle, nodeID, 10000, 500, 10, setpoint, 500, 0, &errorCode)){
			ROS_WARN_STREAM("Node " << nodeID << " could not set home params. Retrying..." );
			if(loopCounter++>maxCommandAttempts){
				ROS_ERROR_STREAM("Could not set Home Parameter: " << getErrorInfo(errorCode));
				return false;
			}
		}
		return true;
	...
	bool findHome(){
		int loopCounter=0;
		while(!VCS_FindHome(keyHandle, nodeID, 23, &errorCode)){
			ROS_WARN_STREAM("Node " << nodeID << " could not find home. Retrying..." );
			if(loopCounter++>maxCommandAttempts){
				ROS_ERROR_STREAM("Could not find Home: " << getErrorInfo(errorCode));
				return false;
			}
		}
		return true;
	...
	bool loadIncEncoderParametersFromEpos(){
		int loopCounter=0;
		while(!VCS_GetIncEncoderParameter(keyHandle, nodeID, (unsigned int*) &sensorIncEncoderResolution,
				&sensorInversion, &errorCode)){
			if (loopCounter++ > maxCommandAttempts){
	...
	bool loadMaxFollowingErrorFromEpos(){
		int loopCounter = 0;
		while (!VCS_GetMaxFollowingError(keyHandle, nodeID, (unsigned int*) &maxFollowingError,
				&errorCode)) {
			if (loopCounter++ > maxCommandAttempts) {
	...
	bool loadVelocityProfileFromEpos(){
		int loopCounter=0;
		while(!VCS_GetVelocityProfile(keyHandle, nodeID, (unsigned int*) &velocityProfileAcceleration, (unsigned int*) &velocityProfileDeceleration, &errorCode)){
			if (loopCounter++ > maxCommandAttempts){
	...
	bool loadPositionProfileFromEpos(){
		int loopCounter=0;
		while(!VCS_GetPositionProfile(keyHandle, nodeID, (unsigned int*) &positionProfileVelocity, (unsigned int*) &positionProfileAcceleration,(unsigned int*) &positionProfileDeceleration, &errorCode)){
			if (loopCounter++ > maxCommandAttempts){
	...
	bool loadMaxAccelerationFromEpos(){
		int loopCounter = 0;
		while (!VCS_GetMaxAcceleration(keyHandle, nodeID,(unsigned int*) &maxAcceleration,
				&errorCode)) {
			if (loopCounter++ > maxCommandAttempts) {
	...
	bool loadMaxVelocityFromEpos(){
		int loopCounter = 0;
		while (!VCS_GetMaxProfileVelocity(keyHandle, nodeID, (unsigned int*)&maxProfileVelocity, &errorCode)) {
			if (loopCounter++ > maxCommandAttempts) {
